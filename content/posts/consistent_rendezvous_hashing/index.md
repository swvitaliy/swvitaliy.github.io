...
Главный недостаток randezvous по сравнению с consistent hashing заключается в необходимости проверки хеша по каждому узлу при вычислении бакета для ключа. В consistent hashing нам достаточно 1 раз вычислить хеш ключика + предварительно вычислить hash nodes/vnodes. Для большого количества узлов для рандеву мы будем иметь линейное увеличение нагрузки для вычисления каждого бакета по ключу. Впрочем, эта нагрузка сильно оптимизируется на уровне процессора для HRW версии (см. бенчмарки ниже).
...
Так же поиск в кольце хешей выполняется через бинпоиск - O(log N) - тогда как в рандеву поиск линеен, что так же может сыграть свою роль при увеличении количества узлов.
...
Интересно так же, что при использовании консистентного хеша в тестах на равномерность распределения пришлось поставить Eps=0.1, тогда как обе рандеву реализации имеют в аналогичных тестах Eps=0.01. Это указывает на то, что даже при использовании vnodes в hash ring равномерность распределения ключей хуже чем для рандеву и прилично отличается от равномерной - до 10% для фактора репликации 3 (количество vnodes на один узел). 

## Применение consistent hashing

В качестве примера применения consistent hashing рассмотрим реализацию выбора узла в 


## Применение randezvous

В качестве примера применения randezvous рассмотрим реализацию 
